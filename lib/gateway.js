    ...
    **/
    ...
    updateIODataStore(channel, json_data) {
        ...
        for (const name in json_data) {
            ...
            const datafield = findDataFieldByID(name);
...
//            json_data = json_data[0];
//
//            if (DATAFIELDS[attr].gateway != null && json_data[DATAFIELDS[attr].gateway] != null) {
//                this.adapter.log.debug('Processing attribute: ' + DATAFIELDS[attr].id);
//                myobj[c_id] = json_data[DATAFIELDS[attr].gateway];
//
//                if (DATAFIELDS[attr].scheduler_conversion != null) {
//                    const exp = parser.parse(DATAFIELDS[attr].gateway_conversion);
    ...
    create_channel(that, obj_id) {
        that.adapter.getObject(obj_id, function (err, obj) {
            if (err || obj == null) {
                ...
                that.adapter.log.info('Creating new gateway channel: ' + obj_id);
