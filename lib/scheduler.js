...
class Scheduler {
    ...
    updateIODataStore() {
        ...
        for (const attr in DATAFIELDS) {
            ...
            if (DATAFIELDS[attr].scheduler != null && this.dataObject[DATAFIELDS[attr].scheduler]) {
                myobj[DATAFIELDS[attr].id] = this.dataObject[DATAFIELDS[attr].scheduler];

                if (DATAFIELDS[attr].scheduler_conversion != null) {
                    const exp = parser.parse(DATAFIELDS[attr].scheduler_conversion);
                    myobj[DATAFIELDS[attr].id] = exp.evaluate({ x: myobj[DATAFIELDS[attr].id] });
                } 
            }
        }
    ...
    setDecimals(objData) {
        const divide_by_10 = [
            ...
            'yearlyrainmax', 'solarradiationmax',
            // abs min values
            ...
            'absbaromdailymax', 'raindailymax', 'solarradiationdailymax',
            // daily min values
