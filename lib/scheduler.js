...
class Scheduler {
    ...
    /**
     * Clears the interval.
     * Falls back to the global timer functions for non-adapter use.
     *
     * @param {NodeJS.Timeout | number} timer The timer to clear
     */
    clearInterval(timer) {
        if (this.adapter && typeof this.adapter.clearInterval === 'function') {
            return this.adapter.clearInterval(timer);
        }
    ...
    start() {
        if (this.interval > 0) {
            this.schedule_timer = this.adapter.setInterval(() => this.getUpdateFromWS(), this.interval * 1000);
        } else {
    ...
    stop() {
        if (this.schedule_timer) {
            this.clearInterval(this.schedule_timer);
        }
    ...
    updateIODataStore(channel, json_data) {
        ...
        const myobj = {};
        // Restrict the parser to basic arithmetic operators only. This limits the
        // expression grammar to what the built-in scheduler_conversion formulas
        // actually need and hardens against arbitrary code execution should the
        // expression definitions in constants.js ever be modified or sourced externally.
