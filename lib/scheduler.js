...
class Scheduler {
    ...
    /**
     * Use adapter-managed timers when available.
    ...
    setInterval(fn, timeout) {
        if (this.adapter && typeof this.adapter.setInterval === 'function') {
            return this.adapter.setInterval(fn, timeout);
        }
    ...
    start() {
        if (this.interval > 0) {
            this.schedule_timer = this.setInterval(() => this.getUpdateFromWS(), this.interval * 1000);
        } else {
