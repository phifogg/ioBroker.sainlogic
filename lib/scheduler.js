...
class Scheduler {
    constructor(address, port, interval, adapter) {
        ...
        this.commands = {getfirmwarecmd,getweatherdatacmd };
    ...
    getUpdateFromWS() {
        ...
        this.dataClient.on('error', this.server_error.bind(this));
...
*/
    ...
    client_connect() {
        ...
        this.run++;
        this.adapter.log.debug('Scheduler connected to weather station run ' + this.run + ' ' + this.commands[this.run]);
        ...
        if (this.run == 1)
            hexVal = new Uint8Array(getfirmwarecmd);
        else
            hexVal = new Uint8Array(getweatherdatacmd);
